...
def ensure_default_debt_loan_auth():
    ...
    rio_users accounts, by explicit request. Single-user tool, so this
    ...
    try:
        existing = col("debt_loan_auth").find_one({"username": "admin"})
        if not existing:
            col("debt_loan_auth").insert_one({
                "username": "admin",
                "password": hash_password("debtloan@123"),
            })
            logger.info("Debt & Loan account created: username=admin password=debtloan@123")
    except Exception as e:
...
app = FastAPI(title="RIO PRINT MEDIA ERP v3.0", lifespan=lifespan)
...
async def serve_mobile(request: Request):
    mobile_file = os.environ.get("MOBILE_FILE", "RIO_MOBILE.html")
    html = _load_html_cached(mobile_file)
    if html is None:
        return HTMLResponse(f"<h2>Mobile file not found: {mobile_file}</h2><p>CWD: {os.getcwd()}</p>", 404)
    return HTMLResponse(html)

@app.get("/stock", response_class=HTMLResponse)
async def serve_stock_mobile(request: Request):
    """Standalone, login-free Stock Ledger tool for production staff — same
    backend/DB as desktop and the main mobile app, just a single-purpose UI."""
    stock_file = os.environ.get("STOCK_MOBILE_FILE", "RIO_STOCK_MOBILE.html")
    html = _load_html_cached(stock_file)
    if html is None:
        return HTMLResponse(f"<h2>Stock mobile file not found: {stock_file}</h2><p>CWD: {os.getcwd()}</p>", 404)
    return HTMLResponse(html)

# ── CMY tools: real server-side protection, admin role only ────────────
# Unlike a password check baked into a standalone HTML file's own
# JavaScript (readable/bypassable by anyone who opens dev tools), this
# verifies the session token against the database on every request, the
# same way any other protected API call would. A token only exists after
# a real login through /api/auth/login, and is checked here server-side —
# not just hidden client-side — so there's no way to reach the tool
# without actually being logged in as admin, regardless of what a person
# does in their own browser.
def verify_admin_token(token: str):
    if not token:
        return None
    user = _db["rio_users"].find_one({"session_token": token}, {"_id": 0})
    if not user:
        return None
...
def verify_user_token(token: str):
    """Same real server-side session check as verify_admin_token, but for
    any logged-in user, not just admins — Renu Contacts is a regular
    feature, not an admin-only tool."""
    if not token:
        return None
    user = _db["rio_users"].find_one({"session_token": token}, {"_id": 0})
    if not user:
...
def _access_denied_page(reason: str):
    return HTMLResponse(f"""<!DOCTYPE html><html><head><meta charset="UTF-8">
<title>Access Denied</title>
<style>body{{background:#16171a;color:#e8e6e1;font-family:'Nunito',sans-serif;
display:flex;align-items:center;justify-content:center;height:100vh;margin:0;text-align:center;}}
.box{{max-width:420px;padding:32px;}}
h1{{font-size:1.3rem;margin-bottom:12px;}}
p{{color:#8a8d94;font-size:0.9rem;line-height:1.6;}}</style></head>
<body><div class="box"><h1>🔒 Access Denied</h1><p>{reason}</p>
<p>Please open this tool from within the Rio ERP (logged in as an admin), not as a direct link.</p></div></body></html>""", status_code=403)
...
async def serve_debt_and_loan(request: Request):
    ...
    rio_users accounts). The page itself loads freely so its login form
...
def list_companies():
    return JSONResponse({"companies":[{"id":"rio","name":"RIO PRINT MEDIA"},{"id":"rainbow","name":"RAINBOW UMBRELLAS"}]})
...
# Own, separate login here (not the main rio_users accounts) - a single
...
# ── SALARY (part-time, calculated from attendance) ─────────────────────────
@app.post("/api/salary")
async def save_salary_record(request: Request):
    if not await ensure_db(): return err("DB offline")
    rec = await request.json()
    try:
        rec.pop("_id", None)
        rec["savedAt"] = datetime.utcnow().isoformat()
        rec["id"] = next_id("salary_records")
        col("salary_records").insert_one(rec)
        return ok({"success": True})
    except Exception as e:
        return err(str(e))
...
async def get_salary_records(
    ...
    if date_from or date_to:
        ...
        query["fromDate"] = rng
    rows = list(col("salary_records").find(query, {"_id": 0}).sort("savedAt", -1))
    return JSONResponse(content=rows)
...
async def login(request: Request):
    ...
    if not username or not password:
        return err("Username and password required", 400)
    user = _db["rio_users"].find_one({"username": username}, {"_id": 0})
    if not user:
        return err("Invalid username or password", 401)
...
async def get_users():
    ...
    users = list(_db["rio_users"].find({}, {"_id": 0, "password": 0, "session_token": 0}))
