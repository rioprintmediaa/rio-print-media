...
async def serve_dashboard(request: Request):
    ...
    with open(HTML_FILE, "r", encoding="utf-8") as f:
        html = f.read()
    # Inject API base URL so browser connects to this server
    api_url = str(request.base_url).rstrip("/")
    inject = f'<script>window.__RIO_API="{api_url}/api";window.__RIO_PORT=null;window.__RIO_ONLINE=true;</script>'
...
async def login(request: Request):
    ...
    if user["password"] != hash_password(password):
        return err("Invalid username or password", 401)
    # Return user info (no persistent token — session managed by browser)
    return JSONResponse(content={
        "ok": True,
