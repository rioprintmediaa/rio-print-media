"""
rio_api.py — RIO PRINT MEDIA Sales Tracker v3.1
...
Fixes from v3.0:
  1. ensure_default_users() moved into lifespan — removed deprecated @app.on_event("startup")
  2. set_sales_ledger_credits() calls next_id() fresh per insert — no manual ID increment
  3. Passwords now use bcrypt (salted) instead of plain SHA-256
  4. serve_dashboard() injection cleaned up — no dead __RIO_PORT injection in cloud mode
  5. next_id() documented with race-condition warning; unique indexes recommended
"""
...
def next_id(collection_name: str, field: str = "Id") -> int:
    """Get next integer Id for a collection.
    NOTE: Not atomic — safe for low-concurrency use.
    For high concurrency, use a MongoDB counters collection with $inc.
    """
    pipeline = [{"$group": {"_id": None, "max": {"$max": f"${field}"}}}]
    res = list(col(collection_name).aggregate(pipeline))
    return (to_int(res[0]["max"]) if res and res[0]["max"] is not None else 0) + 1

# ─────────────────────────────────────────────
#  PASSWORD HASHING  (FIX 3: bcrypt with salt)
# ─────────────────────────────────────────────
def hash_password(password: str) -> str:
    """Hash password with bcrypt (salted). Safe against rainbow tables."""
...
def set_sales_ledger_credits(sno: int, customer: str, job_name: str, payments: list):
    ...
    FIX 2: next_id() called fresh per insert — no manual increment that risks collision.
    """
    ...
    for pay in payments:
        ...
        # FIX 2: get fresh ID each time instead of pre-incrementing ledger_id
        ledger_id = next_id("account_ledger")
        col("account_ledger").insert_one({
            "Id": ledger_id, "AccountName": acct, "EntryDate": dt,
...
def ensure_default_users():
    ...
    if col("rio_users").count_documents({}) == 0:
        col("rio_users").insert_many([
            ...
            {
                "username": "expense1",
                "password": hash_password("expense@1"),
                "role": "expense",
                "name": "Expense User 1"
            },
            {
                "username": "invoice1",
                "password": hash_password("invoice@1"),
                "role": "invoice",
                "name": "Invoice User 1"
            }
        ])
...
# ─────────────────────────────────────────────
#  APP STARTUP  (FIX 1: ensure_default_users inside lifespan, no @on_event)
# ─────────────────────────────────────────────
@asynccontextmanager
async def lifespan(app: FastAPI):
    ...
    # FIX 1: moved here from deprecated @app.on_event("startup")
...
#  FIX 4: removed dead __RIO_PORT injection — patched HTML ignores it anyway
# ─────────────────────────────────────────────
...
@app.get("/api/ping")
async def ping():
    return {"ok": True, "db": MONGO_DB, "server": "MongoDB Atlas"}
...
async def billing_status():
    ...
    return JSONResponse(content={"ready": True, "server": "MongoDB Atlas", "database": MONGO_DB, "version": "3.1", "customers": cc, "products": pc, "invoices": ic})
...
async def login(request: Request):
    ...
    # FIX 3: use bcrypt verify instead of plain hash compare
...
async def get_users():
    users = list(col("rio_users").find({}, {"_id": 0, "password": 0}))
